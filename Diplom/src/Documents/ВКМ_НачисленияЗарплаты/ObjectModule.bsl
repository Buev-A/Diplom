...
Процедура СформироватьДвиженияПрочиеОсновныеНачисления() 
	...
	Пока Выборка.Следующий() Цикл  
		...
		//	Движение.БазовыйПериодКонец = КонецДня(КонецМесяца(Выборка.ДатаОкончания));
		КонецЕсли;
...
Процедура РассчитатьОклад() 
	...
	Пока Выборка.Следующий() Цикл
		...
		СуммаНачислено = Окр(Движение.Показатель * Выборка.Факт / Выборка.Норма,2);
		НДФЛ = Окр((СуммаНачислено * 0.13),2);
...
Процедура РассчитатьУдержания()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ВЫБРАТЬ
	                      |	ВТ_ТабЧасть.Сотрудник КАК Сотрудник,
	                      |	СУММА(ВКМ_ОсновныеНачисления.Показатель) КАК Показатель
	                      |ИЗ
	                      |	ВТ_ТабЧасть КАК ВТ_ТабЧасть
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.ВКМ_ОсновныеНачисления КАК ВКМ_ОсновныеНачисления
	                      |		ПО ВТ_ТабЧасть.Сотрудник = ВКМ_ОсновныеНачисления.Сотрудник
	                      |ГДЕ
	                      |	ВКМ_ОсновныеНачисления.Регистратор = &Регистратор
	                      |
	                      |СГРУППИРОВАТЬ ПО
	                      |	ВТ_ТабЧасть.Сотрудник" );  
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.Показатель - ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.СуммаБаза КАК НДФЛ
	                      |ИЗ
